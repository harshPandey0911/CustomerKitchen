...
export const distributorsApi = {
  list: async () => {
    ...
    try {
      response = await fetch(`${API_BASE_URL}/api/distributors`);
    } catch {
  ...
  create: async (payload) => {
    ...
    if (!response.ok) {
      throw new Error(data.message || 'Unable to create distributor.');
    }
  ...
  update: async (id, payload) => {
    ...
    try {
      response = await fetch(`${API_BASE_URL}/api/distributors/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    if (!response.ok) {
      throw new Error(data.message || 'Unable to update distributor.');
    }
