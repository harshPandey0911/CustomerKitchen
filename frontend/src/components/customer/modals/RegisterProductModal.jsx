...
import ProductPreviewCard from '../register/ProductPreviewCard';
...
const createInitialForm = (productOptions) => ({
...
export default function RegisterProductModal({ open, onClose, productOptions, onSubmit }) {
  const [form, setForm] = useState(() => createInitialForm(productOptions));

  useEffect(() => {
    if (open) {
      setForm(createInitialForm(productOptions));
    }
  }, [open, productOptions]);
  ...
  return (
    <ModalWrapper open={open} title="Register Product" onClose={onClose}>
