import { LuChevronRight, LuPackage, LuPlus } from 'react-icons/lu';
...
function ActionCard({ action, onNavigate }) {
  const Icon = iconMap[action.icon] || LuPackage;
...
export default function AccountSection({ actions, onNavigate }) {
  return (
    <section className="space-y-3">
      <div className="px-1">
        <h2 className="text-[1rem] font-bold tracking-[-0.02em] text-[#1E1E1E]">Account</h2>
        <p className="mt-1 text-sm text-[#6B7280]">Manage your products and registration shortcuts.</p>
      </div>
      <div className="space-y-3">
        {actions.map((action) => (
          <ActionCard key={action.label} action={action} onNavigate={onNavigate} />
        ))}
      </div>
    </section>
  );
